...
```{r}
train_x_no_na["Fate"] <- train_y_no_na
str(train_x_no_na)
```
...
```{r}
SVM_model.train <- svm(Fate ~ Pclass + Sex + Embarked + title + FamilyCat + AgeCat, data = train_x_no_na, probability = TRUE)
```
...
```{r}
SVM_pred.test <- predict(SVM_model.train, test_x_no_na)
```
...
```{r}
CrossTable(test_y_no_na, SVM_pred.test, prop.chisq = FALSE, prop.c = FALSE, prop.r = FALSE, dnn = c("Realitat", "Predicció"))
```

Per avaluar la precisió del model utilitzem una matriu de confusió simple com en l'apartat anterior:

```{r}
matriu_conf_svm <- table(test_y_no_na,Predicted=SVM_pred.test)
